...
Platform.OS == 'ios'?DECOY_IMAGE_SOURCE="/var/mobile/Containers/Data/Application/8B1FCA6B-7A99-4B57-94D7-07DB57A8A72C/Documents/MASKOFF/sample_image.jpg"
                    :DECOY_IMAGE_SOURCE="/data/user/0/maskoff.mosaic/files/MASKOFF/sample_image.png"
...
export default class MASKOFFImageManipulator extends Component {
  ...
  async cancel(){
    try{
      this._canvas.clear()
      base64 = await RNFS.readFile(DECOY_IMAGE_PREFIX + DECOY_IMAGE_SOURCE,"base64")
      extension =  DECOY_IMAGE_SOURCE.split('.').pop();
  ...
  async componentWillMount(){
    try{
      await Image.getSize(DECOY_IMAGE_PREFIX + DECOY_IMAGE_SOURCE, (width, height) => {this.setState({imageWidth:width, imageHeight:height})});
      base64 = await RNFS.readFile(DECOY_IMAGE_PREFIX + DECOY_IMAGE_SOURCE,"base64")
      extension =  DECOY_IMAGE_SOURCE.split('.').pop();
      var path = RNFS.DocumentDirectoryPath + '/MASKOFF_TEMP';
      var temp_filename = path+"/temp_image."+extension
  ...
  async switchToCropRotate(){
    ...
    await this.setState({ modifyPath: temp_filename,page:"crop-rotate"});
    
      
    
    // if(Platform.OS == 'ios'){
    //   result = await RNFS.readDir('/var/mobile/Containers/Data/Application/36694AF6-BF1C-4026-B92C-630C87C0AA67/Documents/MASKOFF_TEMP/')
    //   console.log(result)
    // }else{
    //   result = await RNFS.readDir('/data/user/0/maskoff.mosaic/files/MASKOFF_TEMP/')
    //   console.log(result)
    // }
  ...
  renderCropRotatePage(){
    console.log("renderCropRotatePage",DECOY_IMAGE_PREFIX + this.state.modifyPath)
    return (
      <View style={{ height:height - STATUSBAR_HEIGHT - 50 - 50, flexDirection: 'row' }}>
        <AmazingCropper
          onDone={this.onDone}
          onCancel={this.onCancel}
          imageUri={DECOY_IMAGE_PREFIX + this.state.modifyPath}
          imageWidth={this.state.imageWidth}
          imageHeight={this.state.imageHeight}
