...
import { height, 
         ...
         MAP_STYLE,
         randomColor,
         indexOfMax } from '../utility/Util.js';
...
export class DisplayCard_masker extends React.PureComponent{
  ...
  constructor(props){
    ...
    this.live_until = this.props.info.live_until
  }
  render(){
      ...
      <TouchableOpacity style={DisplayCard_masker_style.card_wrapper}
        ...
        <View style={DisplayCard_masker_style.date_bar}>
          <Text style={DisplayCard_masker_style.date}>
            {Lan["live_until"] + getDateLiveUntil(this.live_until)}
          </Text>
        </View>
...
export class Similarity_bar extends React.PureComponent{
  constructor(props){
    super(props)
    this.sim = this.props.sim
    this.color = ["grey","grey","grey","grey"]
    this.text = Lan["ultra_low"]
    if (this.sim !== undefined){
      if (this.sim > 0.05 && this.sim <= 0.3){
        this.color = ["red","grey","grey","grey"]
        this.text = Lan["low"]
      }
      if(this.sim > 0.3 && this.sim <= 0.66){
        this.color = ["red","red","grey","#ffd300"] 
        this.text = Lan["mid"]
      }else if(this.sim > 0.66 && this.sim <= 0.9){
        this.color = ["red","red","red","red"] 
        this.text = Lan["high"]
      }else if(this.sim > 0.9){
        this.color = ["red","red","red","red"] 
        this.text = Lan["ultra_high"]
      }
    }
  }
  render(){
    return(
      <View style={[Similarity_bar_style.container,{opacity:this.props.highest?1:0.3}]}>
        <View style={[Similarity_bar_style.bar,{backgroundColor:this.color[0]}]}>
        </View>
        <View style={[Similarity_bar_style.bar,{backgroundColor:this.color[1]}]}>
        </View>
        <View style={[Similarity_bar_style.bar,{backgroundColor:this.color[2]}]}>
        </View>
        <Text style={[Similarity_bar_style.text,{color:this.color[3]}]}>
          {this.text}
        </Text>
      </View>
    )
...
const Similarity_bar_style = StyleSheet.create({
  container:{
    flexDirection:"row",
    justifyContent:"flex-start",
    alignItems:"center"
...
export class DisplayCard_masker_search extends React.PureComponent{
  // example
  // <DisplayCard_masker 
  //         info={decoy_masker_1}
  //         />    
  constructor(props){
    ...
    this.contributed = this.props.info.contributed
    this.live_until = this.props.info.live_until
    this.sims = [this.props.info.contacts_similarity.wechat,this.props.info.contacts_similarity.cellphone,this.props.info.contacts_similarity.other,this.props.info.name_similarity]
    this.highest_sim = "name"
    switch(indexOfMax(this.sims)){
      case 0:
        this.highest_sim = "wechat"
      break;
      case 1:
        this.highest_sim = "cellphone"
      break;
      case 2:
        this.highest_sim = "other"
      break;
      case 3:
        this.highest_sim = "name"
      break;
  ...
  render(){
    ...
    return(
      <TouchableOpacity style={DisplayCard_masker_search_style.card_wrapper}
                        onPress={()=>this.props.onPress()}
                        activeOpacity={0.9}>
        <View style={DisplayCard_masker_search_style.title_bar}>
          <Text style={DisplayCard_masker_search_style.title}
                adjustsFontSizeToFit={true}
                numberOfLines={2}
                minimumFontScale={0.01}>
            {this.name}
        ...
        <View style={DisplayCard_masker_search_style.name_similarity_bar}>
          <Text style={DisplayCard_masker_search_style.contacts}>
            {Lan["name_similarity"] + ":"}
          </Text>
          <Similarity_bar sim={this.props.info.name_similarity} highest={this.highest_sim == "name"?true:false}/>
        </View>

        <View style={DisplayCard_masker_search_style.alias_bar}>
          <Text style={DisplayCard_masker_search_style.alias}>
            {this.alias == null?"":this.alias.join(" ")}
          </Text>

        </View>
        <View style={DisplayCard_masker_search_style.tags_bar}>
          <Text style={DisplayCard_masker_search_style.tags}>
            {this.tags.join("# #")}
          </Text>
        </View>
        <View style={DisplayCard_masker_search_style.contacts_bars_wrapper}>
          <View style={DisplayCard_masker_search_style.contacts_bar}>
            <Text style={DisplayCard_masker_search_style.contacts}>
              {Lan["phone_similarity"] + ": "}
            </Text>
            <Similarity_bar sim={this.props.info.contacts_similarity.cellphone} highest={this.highest_sim == "cellphone"?true:false}/>
          </View>
          <View style={DisplayCard_masker_search_style.contacts_bar}>
            <Text style={DisplayCard_masker_search_style.contacts}>
              {Lan["wechat_similarity"]+ ": "}
            </Text>
            <Similarity_bar sim={this.props.info.contacts_similarity.wechat} highest={this.highest_sim == "wechat"?true:false}/>
          </View>
          <View style={DisplayCard_masker_search_style.contacts_bar}>
            <Text style={DisplayCard_masker_search_style.contacts}>
              {this.props.info.contacts.other_key +Lan["similarity"]+ ": "}
            </Text>
            <Similarity_bar sim={this.props.info.contacts_similarity.other} highest={this.highest_sim == "other"?true:false}/>
          </View>
        </View>
        <View style={DisplayCard_masker_search_style.status_bar}>
          <Text style={DisplayCard_masker_search_style.status}>
            <Text style={DisplayCard_masker_search_style.number}> 
              {this.contributed}
            </Text>
             {"人"+Lan["Contributed_count"]}, 
            <Text style={DisplayCard_masker_search_style.number}> 
        ...
        <View style={DisplayCard_masker_search_style.date_bar}>
          <Text style={DisplayCard_masker_search_style.date}>
            {Lan["live_until"] + getDateLiveUntil(this.live_until)}
          </Text>
        </View>
...
const DisplayCard_masker_search_HEIGHT = 150
const DisplayCard_masker_search_style = StyleSheet.create({
  card_wrapper:{
    backgroundColor:BACKGROUND_COLOR,
    width:width,
    flexDirection:"column",
  ...
  tags:{
    fontSize:13,
    color:COLOR_DisplayCard_masker["tags"],
    marginTop:DisplayCard_masker_search_HEIGHT * (25 / 100) / 4,
    marginBottom:DisplayCard_masker_search_HEIGHT * (25 / 100) / 4,
    marginLeft:5,
    //test
    // borderWidth:1
  },
  ...
  alias:{
    fontSize:11,
    marginTop:DisplayCard_masker_search_HEIGHT * (25 / 100) / 8,
    marginBottom:DisplayCard_masker_search_HEIGHT * (25 / 100) / 4,
    color:COLOR_DisplayCard_masker["alias"],
    marginLeft:5,
  ...
  status_bar:{
    ...
    justifyContent:"flex-start",
    backgroundColor:FILLING_COLOR,
    marginBottom:DisplayCard_masker_search_HEIGHT * (2.5 / 100),
    marginLeft:5,
    marginRight:5,
    width:width  - 20,
    borderBottomLeftRadius:3,
  ...
  status:{
    fontSize:DisplayCard_masker_search_HEIGHT * (25 / 100) / 5,
    marginBottom:DisplayCard_masker_search_HEIGHT * (25 / 100) / 10,
    color:COLOR_DisplayCard_masker["status"],
    marginLeft:5,
  ...
  contacts_bars_wrapper:{
    ...
    marginLeft:5,
    marginRight:5,
    paddingBottom:DisplayCard_masker_search_HEIGHT * (25 / 100) / 4,
    width:width  - 20,
    borderTopLeftRadius:3,
    borderTopRightRadius:3,
  },
  ...
  contacts:{
    marginLeft:5,
    color:COLOR_DisplayCard_masker["contacts"],
  }
})
        ...
        <View style={DisplayCard_masker_see_more_style.user_info_bar}>
          <View style={DisplayCard_masker_see_more_style.nickname_time_container}>
            ...
            <Text style={DisplayCard_masker_see_more_style.time}>
              {getDateLiveUntil(this.props.article.live_until)}
            </Text>
          </View>
      ...
      return (
        <TouchableOpacity style={[ContentDisplayPanel_style.display,{flexDirection:'column',
          ...
          <Text style={{opacity:0.5,
              ...
              onPress={() => {this.props.navigation.navigate("MKOF_Image_manipulator",{ImagePicker_uri:this.props.info,usage:"Post_content",info_index:this.props.info_index})}}>{Lan["Edit"]}
          </Text>
      ...
      content = <View style={[ContentDisplayPanel_style.display,{alignSelf: 'center',backgroundColor:TRANSPARENT}]}>
          ...
          >
            <MapboxGL.PointAnnotation
              key={0}
              id={"0"}
              coordinate={[this.props.info.longitude,this.props.info.latitude]}
            >
            </MapboxGL.PointAnnotation>
          </MapboxGL.MapView>
        </View>
...
export class Comment_Block extends Component {
  constructor(props){
    super(props)
    this.state={liked:this.props.comment.user_liked == "True"?true:false}
