...
class Article_Roast_Display extends Component{
  ...
  async _firstConnect(){
    ...
    switch(this.purpose){
      ...
      case "searchRoast":
        ...
        let response = await getComments(global.server + "/comments/" + this.roast.rid + "/ROASTS/" + "0")
        if(response == false){
          //说明前面出现了error, 需要重连
        }else if(response.ok){
          json = await response.json()  
    ...
    switch(true){
      ...
      case (this.state.fetchComments && this.comments.length==0 && !(this.firstCommentsFetchDone == true)):
        return (<TouchableOpacity
                  ...
                  style={Article_Roast_Display_style.loader_container}
                >
                 <Comment_image_spinner key={1}/>
                </TouchableOpacity>)
      break;
      //1 0 1
      case (this.state.fetchComments && !(this.comments.length==0) && this.firstCommentsFetchDone == true):
  ...
  async like(object_type,object_id){
    let response = await Like(object_type,object_id)
    if(response == false){
      //说明前面出现了error, 需要重连
      return false
    }else if(response.ok){
  ...
  render(){
    ...
    if(this.state.loading){
        spinners = []
        if(this.purpose == "searchMasker"){
          spinner_content= <View style={Article_Roast_Display_style.loader_container} key={1}><DisplayCard_masker_full_spinner /></View>
        }else{
          spinner_content = <View style={Article_Roast_Display_style.loader_container} key={1}><DisplayCard_roast_full_spinner /></View>
        }
        spinners.push(<View style={Article_Roast_Display_style.loader_container} key={1}><Comment_image_spinner  key={1}/></View>)
        spinners.push(<View style={Article_Roast_Display_style.loader_container} key={2}><Comment_text_spinner  key={2}/></View>)
        spinners.push(<View style={Article_Roast_Display_style.loader_container} key={3}><Comment_image_spinner  key={3}/></View>)
        spinners.push(<View style={Article_Roast_Display_style.loader_container} key={4}><Comment_text_spinner  key={4}/></View>)
        spinners.push(<View style={Article_Roast_Display_style.loader_container} key={5}><Comment_image_spinner  key={5}/></View>)
        ...
        return(
          <View style={{flex:1,backgroundColor:BACKGROUND_COLOR}}>
            ...
            {ButtonHeader({layout:"searchMasker",
                          ...
                          left_button_onPress:() => {console.log("pressed")},
