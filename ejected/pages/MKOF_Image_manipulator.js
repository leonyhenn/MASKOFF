...
import RNSketchCanvas from '../components/RN_mosaic/index.js';
import AmazingCropper from '../components/RN_cropper/index.js';
var RNFS = require('react-native-fs');
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons';
import { FileSystem } from 'expo';
import { store } from '../store/store.js';
import { updateInfoToPost } from '../store/actions.js';
...
export default class MKOF_Image_manipulator extends Component {
  constructor(props){
    super(props)
    this.state={
  ...
  async cancel(){
    let path = RNFS.DocumentDirectoryPath + '/MASKOFF_TEMP';
    try{
  ...
  async done(){
    let temp_filepath = await this._canvas.save()
    Platform.OS == "ios"? prefex = "file://"
                          :prefex = "file:"
    let new_filepath = prefex + temp_filepath
    switch(this.usage) {
      case "Post_content":
        let info_index = this.props.navigation.getParam("info_index")
        ...
        break;
      case "Composer":
        change_comment_image = this.props.navigation.getParam("change_comment_image")
        await change_comment_image(new_filepath)
        this.props.navigation.goBack()
        break;
      case "Chat":
        onSend = this.props.navigation.getParam("onSend")
        console.log("new_file_name",new_filepath)
        await onSend({image:new_filepath,filename:new_filepath})
        this.props.navigation.goBack()
        break;
    }
  }
  async componentWillMount(){
    try{
      ...
      let base64  = await FileSystem.readAsStringAsync(this.ImagePicker_uri, {encoding:FileSystem.EncodingTypes.Base64})
      
      
      await Image.getSize(this.ImagePicker_uri, (width, height) => {this.setState({imageWidth:width, imageHeight:height})});
      extension =  this.ImagePicker_uri.split('.').pop();
      let path = RNFS.DocumentDirectoryPath + '/MASKOFF_TEMP';
      let temp_filename = path+"/"+uuidv1()+"."+extension
