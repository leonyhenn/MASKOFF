...
class Search_n_Recommend extends Component {
  ...
  renderSpinner(){
    if(this.purpose == "searchRoast" && this.state.isSearching){
      return <View style={Search_n_Recommend_style.loader_container}><DisplayCard_roast_image_spinner  start={"0"} end={"395"}/></View>
    }else if(this.purpose == "searchMasker" && this.state.isSearching){
      return <View style={Search_n_Recommend_style.loader_container}><DisplayCard_masker_text_spinner start={"0"} end={"125"}/></View>
    }
    return null
  }
  ...
  async search(){
    ...
    if(this.purpose == "searchMasker"){
      var response = await SearchMaskers(keyword)
      
    }else{
      var response = await SearchRoasts(keyword)
    } 
  ...
  render() {
    ...
    //如果输入了搜索词,且点击了搜索, 那么显示搜索结果(还未实现)
      ...
      if(this.props.search[1].length == 0 && !this.state.isSearching){
      ...
      }else if(this.props.search[1].length !== 0){
        result_section = <View style={Search_n_Recommend_style.results_wrapper}>
                            <FlatList
                              ...
                              renderItem={({item}) => this.purpose == "searchMasker"?<DisplayCard_masker_search info={item} onPress={() => this.props.navigation.navigate("Article_Roast_Display",{info:item,purpose:"searchMasker"})}/>:<DisplayCard_roast roast={item} onPress={() => this.viewDetail(item)}/>}
        ...
        temp = []
        if(this.purpose == "searchRoast"){
          temp.push(<DisplayCard_roast_image_spinner key={0}/>)
          temp.push(<DisplayCard_roast_image_spinner key={1}/>)
          temp.push(<DisplayCard_roast_image_spinner key={2}/>)
          temp.push(<DisplayCard_roast_image_spinner key={3}/>)
          temp.push(<DisplayCard_roast_image_spinner key={4}/>)
        }else if(this.purpose == "searchMasker"){
        
          temp.push(<DisplayCard_masker_text_spinner key={0}/>)
          temp.push(<DisplayCard_masker_text_spinner key={1}/>)
          temp.push(<DisplayCard_masker_text_spinner key={2}/>)
          temp.push(<DisplayCard_masker_text_spinner key={3}/>)
          temp.push(<DisplayCard_masker_text_spinner key={4}/>)
          temp.push(<DisplayCard_masker_text_spinner key={5}/>)
          temp.push(<DisplayCard_masker_text_spinner key={6}/>)
          temp.push(<DisplayCard_masker_text_spinner key={7}/>)
          temp.push(<DisplayCard_masker_text_spinner key={8}/>)
          temp.push(<DisplayCard_masker_text_spinner key={9}/>)
          temp.push(<DisplayCard_masker_text_spinner key={10}/>)
          temp.push(<DisplayCard_masker_text_spinner key={11}/>)
          temp.push(<DisplayCard_masker_text_spinner key={12}/>)
          temp.push(<DisplayCard_masker_text_spinner key={13}/>)
          temp.push(<DisplayCard_masker_text_spinner key={14}/>)
        }
