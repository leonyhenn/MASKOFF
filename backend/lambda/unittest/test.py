...
delete_all()
(jiangfangzhou_in_db,headers,first_rid) = post_a_roast()
print(headers,first_rid)
parent_comments = []
child_comments = []
for i in range(1):
    c1 = deepcopy(jiangfangzhou_comment(first_rid,False,"0","0",str(i)))
...
for j in range(46):
    c1 = deepcopy(jiangfangzhou_comment(first_rid,False,parent_comments[0],"0","sub"+str(j)))
    r1 = requests.post(url + "/comments",json=c1,headers=headers)
    print(r1.status_code)
    c1_cid = json.loads(r1.content)["cid"]
