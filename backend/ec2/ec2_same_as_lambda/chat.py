...
def chat_login_signiture(client_id):
  ...
  data = "PLACEHOLDER_FOR_GITHUB"+":"+client_id+"::"+ str(timestamp) +":"+ nonce
  print(data)
  data_b = bytes(data,"utf-8")
  signiture = sign(data_b, b'dCss0EjwmIqA2SsATLKzePJC')
  
  return make_response(jsonify({"signature":signiture,
                                "nonce":nonce,
                                "timestamp":timestamp,
                                "msg":data}),200)

# def chat_conversation_signature()
