...
import io
FONT = '/home/ec2-user/lambda/arial.ttf'
...
def upload_multi(content,files):
    ...
    for i in range(len(content)):
        ...
        if ("video" in content[i]["type"].lower()):
            ...
            if(not isinstance(content[i]["filename"],dict)
                or not content[i]['filename']['latitude']
                or not isinstance(content[i]['filename']['latitude'],float)
                or not content[i]['filename']['longitude']
                or not isinstance(content[i]['filename']['longitude'],float)):
                return message(400,"wrong type in location")
            content[i]['filename']['latitude'] = Decimal(str(content[i]['filename']['latitude']))
            content[i]['filename']['longitude'] = Decimal(str(content[i]['filename']['longitude']))
        if(content[i]["type"].lower() == "vote"):
            if(not isinstance(content[i]["filename"],list)
                or not len(content[i]['filename']) >= 2 ):
                return message(400,"wrong type in location")        
    return content
...
# testing purpose code
def test_test_upload():
    
    files = request.files
    # return make_response(jsonify({"message":"done",
    #                               "content":{filename,filetype}}),200)
    file = files["0"]
    result = image_resize_max_2048(file)
    if (not isinstance(result,list)):
        return result
