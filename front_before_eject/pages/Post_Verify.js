...
import { RequestSMSCode } from "../utility/HttpRequests"
...
import { STATUS_BAR_COLOR,
         ...
         TRANSPARENT,
         COLOR_Post_Verify } from '../utility/colors.js'

// UI--checked
export default class Post_Verify extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      callingCode: '86',
  ...
  async send_verification_code(){
    ...
    for (var i=0;i<phoneNum.length;i++){
      if(!numerics.includes(phoneNum[i])){
        ...
        <TouchableWithoutFeedback style={styles.content_container}
          ...
          <View style={styles.content_container}>
            <View style={styles.country_n_phone}>
              ...
              <TextInput
                multiline={false}
                onChangeText={async (num) =>{await this.setState({callingCode:num})}}
              ...
              <TextInput
                multiline={false}
                onChangeText={async (num) => {await this.setState({phoneNum:num})}}
            ...
            <Text style={styles.instructions}>{Lan['Post_Verify_Disclaimer_1']}</Text>
            <Text style={styles.instructions}>{Lan['Post_Verify_Disclaimer_2']}</Text>
            {button}
          </View>
      </TouchableWithoutFeedback>
